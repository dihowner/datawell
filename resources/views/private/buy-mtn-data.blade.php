...
<body class="app sidebar-mini light-mode default-sidebar">
                                ...
                                <div class="card-body">
                                    ...
                                    <form method="post" action="{{ route('user.submit-data-request') }}">
                                        ...
                                        <div class="form-group mb-4">
                                            ...
                                            <select class="form-control form-control-lg bundleCategory"
                                                name="bundleCategory">
                                                <option value="">-- Select --</option>
                                                @if (count($mtnDataCategories) > 0)
                                                    @foreach ($mtnDataCategories as $mtnDataCategory)
                                                        <option value="{{ $mtnDataCategory['category_name'] }}">
                                                            {{ $mtnDataCategory['category_name'] }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>

                                        <div class="form-group mb-4 d-none dataVolume_section">
                                        ...
                                        <button type="submit"
                                            class="btn btn-danger btn-block btn-lg mt-4 mb-0 vendData">
    ...
    <script>
        $(".bundleCategory").on('change', function() {
            ...
            $(".amoutToPay_section").addClass(
                "d-none"); //Hide AMount field incase it has been selected by datavolume
            if (category == "" || category == undefined) {
                $(".dataVolume_section").addClass("d-none");
                swal.fire({
                    icon: "error",
                    title: "Error",
                ...
                let outputField = $(".dataVolume_section");
                getProducts(endpoint, outputField);
            }
        ...
        $(".vendData").click(function(e) {
            ...
            if (load_form) {
                ...
                if (datavolume == '' || phoneNumber == '' || transactPin == '') {
                    swal.fire({
                        icon: "info",
