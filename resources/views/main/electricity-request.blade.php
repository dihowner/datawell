...
<body class="app sidebar-mini light-mode default-sidebar">
                        ...
                        <div class="col-lg-12 col-md-12">
                            <div class="card">
                                <div class="card-header d-flex justify-content-between">
                                    ...
                                    <div class="mb-0">
                                        <h3 class="card-title mb-0">electricity Requests</h3>
                                    </div>
                                ...
                                <div class="card-body p-2">
                                    ...
                                    <div class="table-responsive mb-2">
                                        <table
                                            ...
                                            <tbody>
                                                @if (count($electricityRequests) > 0)
                                                    @php
                                                        $i = 1;
                                                    @endphp
                                                    @foreach ($electricityRequests as $electricityRequest)
                                                        @php
                                                            $productName = isset($electricityRequest->product->product_name) ? $electricityRequest->product->product_name : '';
                                                            $requestId = $electricityRequest->id;
                                                            
                                                            $mobileNigCode = $electricityRequest->mobilenig;
                                                        @endphp
                                                        <tr>
                                                            <td>{{ $i }}</td>
                                                            <td>
                                                                <div class="d-flex align-items-center mb-6">
                                                                    <div class="mr-3"
                                                                        style="background-image: url({{ asset($electricityRequest['image_url']) }}); width: 50px; height: 50px; background-size: cover;">
                                                                    </div>
                                                                    <div>
                                                                        <strong>{{ $productName }}</strong>
                                                                        <div class="d-block">
                                                                            {{ $electricityRequest->product_id }}</div>
                                                                    </div>
                                                                </div>
                                                            </td>
                                                            <td>{{ $mobileNigCode }} </td>
                                                        ...
                                                        <div class="modal fade" id="editRequest{{ $requestId }}"
                                                            ...
                                                            <div class="modal-dialog" role="document">
                                                                <form method="POST"
                                                                    action="{{ route('update-electricity-request', ['id' => $requestId]) }}">
                                                                    @method('PUT')
                                                                    @csrf
                                        ...
                                        @if (count($electricityRequests) > 0)
                                            {{ $electricityRequests->links('components.custom-paginator') }}
                                        @endif
    ...
    @include('components.modals.electricity-request')
        ...
        $(".deleteBtn").on("click", function() {
            ...
            Swal.fire({
                ...
                html: "You want to delete " + productName +
                    " electricity Request code. Purchase of service for " + productName +
                ...
                if (result.isConfirmed) {
                    ...
                    window.location.href = '/main/electricity-request/' + requestId + '/delete';
                } else if (result.dismiss === Swal.DismissReason.cancel) {
                    Swal.fire('Cancelled', 'Your electricity request code is safe', 'info');
                }
            });
