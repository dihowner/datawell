<div class="modal fade" id="createProductModal" tabindex="-1" role="dialog" aria-labelledby="normalmodal"
    ...
    <div class="modal-dialog" role="document">
        <form method="POST" action="{{ route('create-education-request') }}">
            @csrf
                ...
                <div class="modal-body">
                    <div class="form-group mb-2">
                        ...
                        <select class="form-control form-control-lg select2-show-search" name="product_id">
                            @if (count($educationProducts))
                                @foreach ($educationProducts as $educationProduct)
                                    <option value="{{ $educationProduct->product_id }}">
                                        {{ $educationProduct->product_name }}
                                    </option>
                                @endforeach
                            @endif
                        </select>
